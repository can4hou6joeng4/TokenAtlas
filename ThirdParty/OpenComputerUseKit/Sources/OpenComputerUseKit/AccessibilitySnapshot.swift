...
public struct ComputerUseElementDescriptor: Sendable, Equatable {
    public let index: Int
    public let role: String
    public let renderedLine: String
    public let actions: [String]
    public let frame: CGRect?
    public let isSyntheticText: Bool
...
public struct ComputerUseSnapshotConfiguration: Sendable {
    public var allowsWindowRecovery: Bool
    public var includesScreenshot: Bool

    public init(allowsWindowRecovery: Bool = true, includesScreenshot: Bool = true) {
        self.allowsWindowRecovery = allowsWindowRecovery
        self.includesScreenshot = includesScreenshot
    }

    public static let `default` = ComputerUseSnapshotConfiguration()
    public static let nonIntrusive = ComputerUseSnapshotConfiguration(
        allowsWindowRecovery: false,
...
public struct AppSnapshot {
    ...
    public var elementDescriptors: [ComputerUseElementDescriptor] {
        let linesByIndex = Dictionary(uniqueKeysWithValues: treeLines.compactMap { line -> (Int, String)? in
            guard let index = renderedElementIndex(in: line) else { return nil }
            return (index, line.trimmingCharacters(in: .whitespacesAndNewlines))
        })

        return elements.values.sorted { $0.index < $1.index }.map { record in
            let line = linesByIndex[record.index] ?? ""
            let role = record.element.flatMap { stringValue(of: $0, attribute: kAXRoleAttribute) }
                ?? renderedRole(in: line)
                ?? ""
            return ComputerUseElementDescriptor(
                index: record.index,
                role: role,
                renderedLine: line,
                actions: record.prettyActions,
                frame: record.localFrame,
...
enum SnapshotBuilder {
    static func build(
        ...
        configuration: ComputerUseSnapshotConfiguration = .default
    ) throws -> AppSnapshot {
        ...
        if focusedWindow == nil,
           configuration.allowsWindowRecovery,
        ...
        var windowCapture = WindowCapture.resolve(
            ...
            includeImage: configuration.includesScreenshot
        )
    ...
    private static func buildAccessibilitySnapshot(
        ...
        configuration: ComputerUseSnapshotConfiguration
    ) -> AppSnapshot {
        let windowBounds = windowCapture.bounds
        let screenshotPNGData = windowCapture.pngDataIfAvailable(includeScreenshot: configuration.includesScreenshot)
...
private struct WindowCapture {
    ...
    static func resolve(for pid: pid_t, titleHint: String?, includeImage: Bool = true) -> WindowCapture? {
        ...
        let image = includeImage ? captureImage(windowID: best.windowID, bounds: best.bounds) : nil
    ...
    func pngDataIfAvailable(includeScreenshot: Bool = true) -> Data? {
...
private func renderedElementIndex(in line: String) -> Int? {
    let trimmed = line.trimmingCharacters(in: .whitespacesAndNewlines)
    guard let firstToken = trimmed.split(whereSeparator: \.isWhitespace).first else {
        return nil
    }
    return Int(firstToken)
}

private func renderedRole(in line: String) -> String? {
    let trimmed = line.trimmingCharacters(in: .whitespacesAndNewlines)
    let pieces = trimmed.split(whereSeparator: \.isWhitespace)
    guard pieces.count >= 2, Int(pieces[0]) != nil else {
        return nil
    }
